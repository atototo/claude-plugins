...
const result = {
  systemMessage: [
    ...
    "5. **Session 초기화 (Leader 스폰 전 필수)**:",
    "   Bash: node \"${CLAUDE_PLUGIN_ROOT}/lib/session-cli.mjs\" init --team \"{team}\" --task \"{task}\" --members '[{\"name\":\"leader\",\"agent\":\"leader-agent\",\"role\":\"orchestrator\"},{...workers}]'",
    "   이 명령이 .party/, .party/findings/, .party/tickets/ 디렉토리와 session.json을 생성한다.",
