...
const instruction = [
  ...
  "     · 사용자 요청 원문",
  "     · 팀 구성 (역할별 에이전트 목록)",
  "     · 'CONTEXTUALIZING 단계 수행 필수: (1) cat .party/session.json으로 phase 확인, (2) Grep/Glob/Read로 관련 파일 탐색, (3) .party/findings/context.md 작성. 작성 완료 즉시 훅이 자동으로 다음 phase로 전환한다.'",
  "     · 'context.md 작성 완료 후 SendMessage로 워커에게 작업을 배정하라. 모든 워커는 이미 스폰되어 대기 중이다.'",
  "     · 'Agent 도구를 직접 호출하지 마라.'",
  "   - Leader prompt에 포함하면 안 되는 내용:",
  "     · 'Spawn X agent' 같은 스폰 지시 (Leader는 에이전트 스폰 권한이 없다)",
  "     · 구체적인 단계별 작업 지시 (context.md 작성 전에는 배정 금지)",
  "7. **즉시 연속으로** 팀 프리셋의 모든 worker를 역할 기반으로 스폰하라:",
  "   - 역할명을 subagent_type에 사용: ai-party:{role} (예: ai-party:architect, ai-party:analyst, ai-party:builder)",
  "   - 팀 프리셋에서 '{agent} as {role}' 형태면, {role}을 subagent_type에 사용하라",
  "   - 모델 티어링: opus(architect, reviewer, security-auditor), sonnet(analyst, builder, researcher, deployer)",
  "   ⚠️ Worker prompt 규칙 (반드시 준수):",
  "     · prompt에 task 지시를 포함하지 마라. task는 leader가 SendMessage로 전달한다.",
  "     · prompt 템플릿: 'You are {name} ({role}) in team {team_name}. Wait for leader SendMessage before starting any work. Do NOT start from this spawn prompt. Report completion to leader via SendMessage(recipient=\"leader\").'",
  "   - Agent(subagent_type='ai-party:architect', team_name=<위 team_name>, name='architect', description='Architecture design worker', mode='bypassPermissions', model='opus', max_turns=15, prompt='<위 템플릿 적용>')",
  "   - Agent(subagent_type='ai-party:builder', team_name=<위 team_name>, name='builder', description='Implementation builder worker', mode='bypassPermissions', model='sonnet', max_turns=8, prompt='<위 템플릿 적용>')",
  "   - Agent(subagent_type='ai-party:analyst', team_name=<위 team_name>, name='analyst', description='Analysis and scanning worker', mode='bypassPermissions', model='sonnet', max_turns=8, prompt='<위 템플릿 적용>')",
