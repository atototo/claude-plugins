...
const TRANSITIONS = {
  ...
  [STATES.CONTEXTUALIZING]: new Set([STATES.ANALYZING, STATES.PLANNING, STATES.PENDING_APPROVAL]),
  // Team별 워크플로우 예외 지원:
  // - review team: ANALYZING → REVIEWING
  // - research/security team: PLANNING → AWAITING_APPROVAL
  // - 실행 생략 팀: ANALYZING/PLANNING/EXECUTING에서 바로 다음 유효 phase로 이동
  [STATES.ANALYZING]: new Set([STATES.PLANNING, STATES.REVIEWING, STATES.AWAITING_APPROVAL, STATES.PENDING_APPROVAL]),
  [STATES.PLANNING]: new Set([STATES.EXECUTING, STATES.REVIEWING, STATES.AWAITING_APPROVAL, STATES.PENDING_APPROVAL]),
  [STATES.EXECUTING]: new Set([STATES.REVIEWING, STATES.AWAITING_APPROVAL, STATES.PENDING_APPROVAL]),
  [STATES.REVIEWING]: new Set([STATES.AWAITING_APPROVAL, STATES.PENDING_APPROVAL]),
  [STATES.PENDING_APPROVAL]: new Set([
